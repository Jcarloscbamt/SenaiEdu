<div class="max-w-xl mx-auto mt-12 p-6 bg-white rounded-lg shadow-md">
    <h2 class="text-xl font-semibold mb-4 text-center">Cadastro de Feriados</h2>
            ...
            <tbody>
                @foreach($feriados as $feriado)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-2">{{ $feriado->id }}</td>
                        <td class="p-2">{{ $feriado->nome }}</td>
                        <td class="p-2">{{ \Carbon\Carbon::parse($feriado->data)->format('d/m/Y') }}</td>
                        <td class="p-2 flex justify-center gap-1">
                            <button wire:click="edit({{ $feriado->id }})"
                                class="px-2 py-1 bg-yellow-400 text-white rounded hover:bg-yellow-500 text-xs">
                                Editar
                            </button>
                            <button wire:click="delete({{ $feriado->id }})"
